...
async def test_stream_mode(mode: str, description: str):
    ...
    try:
        ...
        async for chunk in agent._graph.astream(
            {"messages": dump_messages(messages), "session_id": f"test-{mode}"}, config, stream_mode=mode
        ):
            chunk_count += 1
...
async def test_custom_streaming():
    ...
    try:
        ...
        async for event in agent._graph.astream_events(
            {"messages": dump_messages(messages), "session_id": "test-custom"}, config, version="v2"
        ):
            chunk_count += 1
            elapsed = time.time() - start_time

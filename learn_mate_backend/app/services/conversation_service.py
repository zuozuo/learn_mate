...
class ConversationService:
    ...
    async def get_conversations(
        ...
        """
        conversations, total = self.conversation_repo.get_conversations_by_user(
    ...
    async def delete_conversation(
        ...
        """
        return self.conversation_repo.soft_delete_conversation(
            conversation_id, user_id
        )
    ...
    async def add_message(
        ...
        if not conversation:
            logger.warning("unauthorized_message_attempt", 
        ...
        if conversation.title == "New Conversation" and role == MessageRole.USER:
            ...
            if message_count <= 2:  # First user message
                ...
                self.conversation_repo.update_conversation(
                    conversation_id, user_id, title=new_title
                )

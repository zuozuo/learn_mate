...
class TestConversationFlow:
    ...
    def test_complete_conversation_flow(
        self, client: TestClient, auth_headers: dict, mock_langgraph_agent, session: Session
    ):
        ...
        create_response = client.post(
            ...
            json={"title": "Integration Test Conversation", "first_message": "Hello, AI!"},
        )
        ...
        detail_response = client.get(f"/api/v1/conversations/{conversation_id}", headers=auth_headers)
        ...
        update_response = client.patch(
            f"/api/v1/conversations/{conversation_id}", headers=auth_headers, json={"title": "Python Discussion"}
        )
        ...
        final_list = client.get("/api/v1/conversations", headers=auth_headers).json()["conversations"]
        assert not any(c["id"] == conversation_id for c in final_list)

    @pytest.mark.integration
    def test_streaming_conversation_flow(self, client: TestClient, auth_headers: dict, mock_langgraph_agent):
        """Test streaming message flow."""
        # Create conversation
        create_response = client.post("/api/v1/conversations", headers=auth_headers, json={"title": "Streaming Test"})
        ...
        with client.stream(
            ...
            headers=auth_headers,
            json={"messages": [{"role": "user", "content": "Stream this response"}]},
        ) as response:
    ...
    @pytest.mark.integration
    def test_multiple_users_isolation(self, client: TestClient, session: Session, mock_langgraph_agent):
        ...
        for i, header in enumerate(headers):
            response = client.post("/api/v1/conversations", headers=header, json={"title": f"User {i} Conversation"})
        ...
        for i, header in enumerate(headers):
            ...
            response = client.get(f"/api/v1/conversations/{other_id}", headers=header)
            assert response.status_code == 404

    @pytest.mark.integration
    @pytest.mark.skip(reason="Thinking content extraction requires complex mock setup")
    def test_thinking_content_persistence(self, client: TestClient, auth_headers: dict, session: Session, monkeypatch):
        ...
        conv_response = client.post("/api/v1/conversations", headers=auth_headers, json={"title": "Thinking Test"})
        ...
        detail_response = client.get(f"/api/v1/conversations/{conv_id}", headers=auth_headers)
    ...
    def test_conversation_search_and_pagination(
        ...
        response = client.get("/api/v1/conversations?search=Python&page=1&limit=3", headers=auth_headers)
        ...
        # Get second page
        response = client.get("/api/v1/conversations?search=Python&page=2&limit=3", headers=auth_headers)

...
class TestAuthAPI:
    ...
    def test_register_duplicate_email(self, client: TestClient, test_user: User):
        ...
        response = client.post("/api/v1/auth/register", json={"email": test_user.email, "password": "AnotherPass123!"})
    ...
    def test_register_weak_password(self, client: TestClient):
        ...
        response = client.post("/api/v1/auth/register", json={"email": "weak@example.com", "password": "weak"})
    ...
    def test_login_wrong_password(self, client: TestClient, test_user: User):
        ...
        response = client.post(
            "/api/v1/auth/login",
            data={"username": test_user.email, "password": "wrongpassword", "grant_type": "password"},
        )
    ...
    def test_login_nonexistent_user(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/auth/login",
            data={"username": "nonexistent@example.com", "password": "password123", "grant_type": "password"},
        )

        assert response.status_code == 401
        assert "Incorrect email or password" in response.json()["detail"]

    def test_login_invalid_grant_type(self, client: TestClient, test_user: User):
        """Test login with invalid grant type."""
        response = client.post(
            "/api/v1/auth/login",
            data={"username": test_user.email, "password": "testpassword123", "grant_type": "client_credentials"},
        )

        assert response.status_code == 400
        assert "Unsupported grant type" in response.json()["detail"]

    def test_protected_endpoint_with_valid_token(self, client: TestClient, test_user: User, auth_headers: dict):
    ...
    def test_protected_endpoint_with_expired_token(self, client: TestClient, test_user: User):
        ...
        payload = {"sub": str(test_user.id), "exp": datetime.utcnow() - timedelta(hours=1)}

        expired_token = jwt.encode(payload, os.getenv("JWT_SECRET_KEY", "test_secret"), algorithm="HS256")
    ...
    def test_get_current_user(self, client: TestClient, session: Session, test_user: User):

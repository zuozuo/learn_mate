...
class TestConversationsAPI:
    ...
    def test_create_conversation_success(self, client: TestClient, auth_headers: dict, test_user: User):
        """Test successful conversation creation."""
        response = client.post("/api/v1/conversations", headers=auth_headers, json={"title": "Test Conversation"})
    ...
    def test_create_conversation_with_first_message(self, client: TestClient, auth_headers: dict, session: Session):
        """Test creating conversation with initial message."""
        response = client.post(
            "/api/v1/conversations",
            headers=auth_headers,
            json={"title": "Chat with first message", "first_message": "Hello, world!"},
        )
        ...
        messages = session.query(ChatMessage).filter(ChatMessage.conversation_id == UUID(conversation_id)).all()
    ...
    def test_get_conversations_with_pagination(
        ...
        # Test first page
        response = client.get("/api/v1/conversations?page=1&limit=3", headers=auth_headers)
    ...
    def test_get_conversations_with_search(
        ...
        # Search for Python
        response = client.get("/api/v1/conversations?search=Python", headers=auth_headers)
    ...
    def test_get_conversation_detail_success(
        self, client: TestClient, auth_headers: dict, test_conversation: Conversation, test_messages: list[ChatMessage]
    ):
        """Test retrieving conversation with messages."""
        response = client.get(f"/api/v1/conversations/{test_conversation.id}", headers=auth_headers)
    ...
    def test_get_conversation_detail_not_found(self, client: TestClient, auth_headers: dict):
    ...
    def test_get_conversation_detail_unauthorized(
        ...
        response = client.get(f"/api/v1/conversations/{other_conversation.id}", headers=auth_headers)

        assert response.status_code == 404

    def test_update_conversation_title(self, client: TestClient, auth_headers: dict, test_conversation: Conversation):
        """Test updating conversation title."""
        new_title = "Updated Title"
        response = client.patch(
            f"/api/v1/conversations/{test_conversation.id}", headers=auth_headers, json={"title": new_title}
        )
    ...
    def test_update_conversation_not_found(self, client: TestClient, auth_headers: dict):
        ...
        fake_id = uuid4()
        response = client.patch(f"/api/v1/conversations/{fake_id}", headers=auth_headers, json={"title": "New Title"})

        assert response.status_code == 404

    def test_delete_conversation_success(
        self, client: TestClient, auth_headers: dict, test_conversation: Conversation, session: Session
    ):
        """Test soft deleting a conversation."""
        response = client.delete(f"/api/v1/conversations/{test_conversation.id}", headers=auth_headers)
    ...
    def test_delete_conversation_not_found(self, client: TestClient, auth_headers: dict):
        ...
        fake_id = uuid4()
        response = client.delete(f"/api/v1/conversations/{fake_id}", headers=auth_headers)

...
async def test_langgraph_streaming():
    ...
    try:
        async for chunk in agent._graph.astream(
            {"messages": dump_messages(messages), "session_id": "test-debug"}, 
            config, 
    ...
    try:
        async for chunk in agent._graph.astream(
            {"messages": dump_messages(messages), "session_id": "test-debug2"}, 
            config
        ):
            chunk_count += 1
            if chunk_count <= 3:
                print(f"   Chunk {chunk_count}: type={type(chunk)}, keys={list(chunk.keys()) if isinstance(chunk, dict) else 'N/A'}")
        print(f"   Total chunks: {chunk_count}")
    except Exception as e:
        print(f"   Error: {e}")
    ...
    if hasattr(agent._graph, 'astream_events'):
        ...
        try:
            async for event in agent._graph.astream_events(
                {"messages": dump_messages(messages), "session_id": "test-debug3"}, 
                config,
                version="v2"
            ):
                event_count += 1

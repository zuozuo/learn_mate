...
async def test_message_structure():
    ...
    async for msg, metadata in agent._graph.astream(
        {"messages": dump_messages(messages), "session_id": "test-structure"}, config, stream_mode="messages"
    ):
        chunk_count += 1
...
async def test_token_streaming():
    ...
    async for msg, metadata in agent._graph.astream(
        {"messages": dump_messages(messages), "session_id": "test-tokens"}, config, stream_mode="messages"
    ):
...
async def test_multiple_modes():
    ...
    async for mode, data in agent._graph.astream(
        {"messages": dump_messages(messages), "session_id": "test-multi"}, config, stream_mode=["messages", "updates"]
    ):

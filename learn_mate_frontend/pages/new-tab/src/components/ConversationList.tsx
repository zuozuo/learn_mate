    ...
    return (
      <div className={cn('conversation-list', !collapsed && (isLight ? 'bg-gray-50' : 'bg-gray-900'))}>
        ...
        <div className="conversations-container">
          {loading ? (
            <div className={cn('loading-state', isLight ? 'text-gray-600' : 'text-gray-400')}>
              <div className="spinner"></div>
              <span>Loading conversations...</span>
            </div>
          ) : conversations.length === 0 ? (
            <div className={cn('empty-state', isLight ? 'text-gray-500' : 'text-gray-400')}>
              <MessageSquare size={32} />
              <p>No conversations yet</p>
              <p className={cn('empty-state-hint', isLight ? 'text-gray-400' : 'text-gray-500')}>
            ...
            Object.entries(conversationGroups).map(([dateLabel, convs]) => (
              <div key={dateLabel} className="conversation-group">
                <div className={cn('group-label', isLight ? 'text-gray-600' : 'text-gray-400')}>{dateLabel}</div>
                {convs.map(conversation => (
                  <button
                    ...
                    className={cn(
                      ...
                      currentConversationId === conversation.id && (isLight ? 'bg-white shadow-sm' : 'bg-gray-800'),
                    )}
                    onClick={() => onSelectConversation(conversation.id)}>
                    <MessageSquare
                      size={16}
                      className={cn('conversation-icon', isLight ? 'text-gray-500' : 'text-gray-400')}
                    />
                    <div className="conversation-content">
                      ...
                      {conversation.message_count !== undefined && (
                        <div className={cn('conversation-meta', isLight ? 'text-gray-500' : 'text-gray-400')}>
